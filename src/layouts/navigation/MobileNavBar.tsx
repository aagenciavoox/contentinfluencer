...
export function MobileNavBar() {
  ...
  const currentBook = (
    (preferredBookId
      ? state.bibliotecaItems.find(book => book.id === preferredBookId)
      : null)
    ?? state.bibliotecaItems.find((book) => ['Consumindo', 'Lendo'].includes(book.status))
  );
  ...
  const actions = [
    ...
    },
    {
      ...
      helperText: currentBook ? currentBook.titulo : 'Sem item principal definido',
    },
  ];
                    ...
                    {action.label === 'Nova Anotação' && currentBook && (
                      <div className="ml-3 flex shrink-0 items-center gap-2 rounded-full border border-[var(--border-color)] bg-[var(--bg-hover)] px-3 py-1">
                        ...
                        <span className="t-secondary max-w-[80px] truncate">
                          {preferredBookId === currentBook.id ? 'Principal' : 'Ativo'}
                        </span>
                      </div>
                    )}
                  </button>
