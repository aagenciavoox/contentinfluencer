  ...
  const sections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {to: '/gravacao', label: 'Gravação', icon: Camera},
      ],
    },
    {
      title: 'organização',
  ...
  const renderNavItem = ({to, label, icon: Icon, badge}: NavItem) => (
    ...
    >
      {({isActive}) => (
        <>
          {isActive ? (
            <span className="absolute left-0 top-2 bottom-2 w-[3px] rounded-full bg-[#4c63ff]" />
          ) : null}
          <div
            className={cn(
    ...
    <div className="flex h-full w-full flex-col border-r border-[#edf0f6] bg-[linear-gradient(180deg,#fdfdff_0%,#fbfbfd_100%)] px-4 py-4 text-[#1f2430] md:px-4 md:py-4">
      <div className="mb-5 flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-[1rem] font-black uppercase tracking-[0.34em] text-[#202635]">
              Core
            </p>
