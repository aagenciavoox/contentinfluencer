...
import { X, Plus, Trash2, Lightbulb, CheckCircle2, BookOpen, MessageSquare } from 'lucide-react';
...
const TIPOS = ['Anotação', 'Trecho', 'Reação', 'Análise', 'Ideia de conteúdo', 'Pergunta'];
  ...
  return (
    <BottomSheetModal open={true} onClose={onClose} desktopMaxW="max-w-4xl" zIndex="z-[100]">
      <div className="flex items-center justify-between p-5 md:p-8 border-b border-[var(--border-color)] bg-[var(--bg-secondary)] shrink-0">
        ...
        <button onClick={onClose} className="p-3 hover:bg-[var(--bg-hover)] rounded-full transition-all text-[var(--text-primary)] opacity-40 hover:opacity-100">
          <X className="w-6 h-6" />
        </button>
      </div>
        ...
        <div className="space-y-6 mt-4">
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <span className="text-[9px] font-black text-[var(--text-tertiary)] opacity-50 uppercase tracking-widest whitespace-nowrap shrink-0">Filtros</span>
              ...
              {anotacoesFiltradas.map(a => (
                ...
                >
                  <div className="flex items-center gap-2 mb-4">
                    <span className={cn("text-[8px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full", TIPO_CORES[a.tipo] || TIPO_CORES['Anotação'])}>
                      {a.tipo}
                    </span>
                    {a.capituloRef && (
