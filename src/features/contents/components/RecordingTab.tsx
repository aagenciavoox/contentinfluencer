...
function getMetaLabel(value: string | null | undefined, fallback: string) {
...
function RecordingBlockCard({ block, contents, onOpen, onDelete }: RecordingBlockCardProps) {
  const { firstContent, completedCount, isCompleted, progressPercentage, totalCount } = getBlockSummary(contents);
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 border-t border-[var(--border-color)] pt-6">
        ...
        <div className="space-y-2">
          ...
          <p className="truncate text-xs font-bold text-[var(--text-primary)]">
            {getMetaLabel(firstContent?.lookId, 'Não definido')}
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="truncate text-xs font-bold text-[var(--text-primary)]">
            {getMetaLabel(firstContent?.cenarioId, 'Não definido')}
          </p>
        </div>
      </div>
    </div>
      ...
      <div className="custom-scrollbar flex-1 overflow-y-auto divide-y divide-[var(--border-color)] p-6 md:p-10">
        {contents.length === 0 ? (
          ...
          contents.map((content, index) => {
            const isDone = content.status !== READY_STATUS;
                  ...
                  <div className="mb-4 flex flex-wrap items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">
                    {content.lookId && (
                      <span className="flex min-w-0 items-center gap-1">
                        ...
                        <span className="truncate">{content.lookId}</span>
                      </span>
                    )}
                    {content.cenarioId && (
                      <span className="flex min-w-0 items-center gap-1">
                        <MapPin className="h-3 w-3 shrink-0" />
                        <span className="truncate">{content.cenarioId}</span>
                      </span>
                    )}
                  </div>
      ...
      <header className="relative flex shrink-0 items-center justify-between border-b border-[var(--border-color)] bg-[var(--bg-secondary)] p-4 md:p-8">
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="flex items-center gap-2">
            <div className="h-1.5 w-1.5 rounded-full bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.8)] animate-pulse" />
