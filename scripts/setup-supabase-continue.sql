-- Continuar migração: seção 2 (ALTER) + seção 3/4 (tabelas novas + RLS) + foundation.
-- Idempotente. Rode o arquivo inteiro no SQL Editor.
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'contents' AND column_name = 'pillar'
  ) THEN
    UPDATE public.contents SET pilar_id = pillar WHERE pilar_id IS NULL AND pillar IS NOT NULL;
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'contents' AND column_name = 'scenario'
  ) THEN
  ...
  IF EXISTS (
    ...
    UPDATE public.contents SET biblioteca_item_id = livro_origem_id
    WHERE biblioteca_item_id IS NULL AND livro_origem_id IS NOT NULL;
  END IF;
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'contents' AND column_name = 'references'
  ) THEN
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'agenda_items' AND column_name = 'type'
  ) THEN
