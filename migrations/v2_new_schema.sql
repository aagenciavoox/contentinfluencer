-- ============================================================================
-- MIGRAÇÃO v2: Schema novo para Content OS
-- ============================================================================
...
-- dna_voz: adicionar id e updated_at, remover coluna pilares não usada
...
-- pilares: adicionar created_at e updated_at, remover colunas substituídas por pilar_plataformas
...
-- series: adicionar created_at e updated_at, remover colunas substituídas por junction tables
...
-- looks: renomear cenario_associado_id → cenario_id, adicionar created_at
DO $$
BEGIN
  IF EXISTS (
...
-- Migrar dados: pillar → pilar_id, scenario → cenario_id, livro_origem_id → biblioteca_item_id
...
-- Converter tags de text para text[] (se necessário)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
...
-- recording_blocks: adicionar colunas novas, remover content_ids (substituído por junction table)
...
-- agenda_items: adicionar novas colunas, migrar type → tipo, remover antigas
...
-- Migrar type → tipo onde fizer sentido
UPDATE public.agenda_items
  SET tipo = CASE
    WHEN type IN ('Reunião', 'Entrega', 'Publicação', 'Outro') THEN type
    ELSE 'Outro'
  END
